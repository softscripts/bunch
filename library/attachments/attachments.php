...
$current_url = get_bloginfo(template_url)."/library/attachments";
$current_dir = STYLESHEETPATH."/library/attachments";

define( 'ATTACHMENTS_PREFIX',   'attachments_' );
define( 'ATTACHMENTS_VERSION',  '1.6.2.1' );
define( 'ATTACHMENTS_URL',      $current_url );
define( 'ATTACHMENTS_DIR',     $current_dir );
    ...
    <li class="attachments-file">
        <h2>
            <a href="#" class="attachment-handle">
                <span class="attachment-handle-icon"><img src="<?php echo ATTACHMENTS_URL; ?>/images/handle.gif" alt="Drag" /></span>
            </a>
        ...
        <div class="attachments-fields">
            <div class="textfield" id="field_attachment_title_<?php echo empty( $id ) ? '{{id}}' : $id; ?>">
                ...
                <input type="text" id="attachment_title_<?php echo empty( $id ) ? '{{id}}' : $id; ?>" name="attachment_title_<?php echo empty( $id ) ? '{{id}}' : $id; ?>" value="<?php echo empty( $title ) ? '' : $title; ?>" size="20" />
            </div>
            <div class="textfield" id="field_attachment_caption_<?php echo empty( $id ) ? '{{id}}' : $id; ?>">
                ...
                <input type="text" id="attachment_caption_<?php echo empty( $id ) ? '{{id}}' : $id; ?>" name="attachment_caption_<?php echo empty( $id ) ? '{{id}}' : $id; ?>" value="<?php echo empty( $caption ) ? '' : $caption; ?>" size="20" />
            </div>
        </div>
...
function attachments_meta_box()
{
    ...
    if( function_exists( 'get_post_types' ) )
    {
        ...
        foreach($post_types as $post_type)
        {
            ...
            {
                add_meta_box( 'attachments_list', __( 'Gallery', 'attachments' ), 'attachments_add', $post_type->name, 'normal' );
            }
        }
    }
}
...
function attachments_init_js()
{
    ...
    echo '  var attachments_base = "' . ATTACHMENTS_URL . '"; ';
